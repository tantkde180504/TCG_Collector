            ...
            actions: [
              ...
              ElevatedButton(
                ...
                onPressed: cartVM.isLoading
                    ? null
                    : () async {
                        final success = await cartVM.verifyPayOSPayment(order.orderId);
                        if (!context.mounted) return;
                        
                        if (success) {
                          Navigator.of(context).pop(); // Close dialog
                          
                          // Retrieve latest order state
                          final updatedOrder = cartVM.orders.firstWhere(
                          ...
                          notifVM.addNotification(
                            ...
                            body: 'Your order ${order.orderId} of $amountVndStr ₫ has been successfully paid.',
                            type: 'order_status',
                          );
                          
                          cartVM.setCheckoutStep(2); // Go to receipt
                        } else {
